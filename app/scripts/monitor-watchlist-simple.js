#!/usr/bin/env ts-node
"use strict";
/**
 * Simple Watchlist Monitor Script
 * Queries the database directly to check watchlist status
 */
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
var client_1 = require("@prisma/client");
var prisma = new client_1.PrismaClient();
function monitorWatchlist() {
    return __awaiter(this, void 0, void 0, function () {
        var timestamp, watchlistItems, alertsCount, oneDayAgo, recentAlerts, results, error_1;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    timestamp = new Date().toISOString();
                    console.log("[".concat(timestamp, "] Starting watchlist monitoring..."));
                    _a.label = 1;
                case 1:
                    _a.trys.push([1, 5, 6, 8]);
                    return [4 /*yield*/, prisma.watchlistItem.findMany({
                            include: {
                                user: {
                                    select: {
                                        id: true,
                                        email: true,
                                        telegramUsername: true,
                                        telegramChatId: true,
                                        isPremium: true,
                                        trialEndsAt: true
                                    }
                                }
                            },
                            orderBy: {
                                createdAt: 'desc'
                            }
                        })];
                case 2:
                    watchlistItems = _a.sent();
                    return [4 /*yield*/, prisma.transactionAlert.count()];
                case 3:
                    alertsCount = _a.sent();
                    oneDayAgo = new Date(Date.now() - 24 * 60 * 60 * 1000);
                    return [4 /*yield*/, prisma.transactionAlert.findMany({
                            where: {
                                createdAt: {
                                    gte: oneDayAgo
                                }
                            },
                            include: {
                                user: {
                                    select: {
                                        email: true
                                    }
                                }
                            },
                            orderBy: {
                                createdAt: 'desc'
                            },
                            take: 50
                        })];
                case 4:
                    recentAlerts = _a.sent();
                    results = {
                        timestamp: timestamp,
                        success: true,
                        walletsChecked: watchlistItems.length,
                        totalAlerts: alertsCount,
                        recentAlerts: recentAlerts.length,
                        watchlistItems: watchlistItems.map(function (item) { return ({
                            id: item.id,
                            address: item.address,
                            chain: item.chain,
                            tokenAddress: item.tokenAddress,
                            tokenSymbol: item.tokenSymbol,
                            lastChecked: item.lastChecked,
                            createdAt: item.createdAt,
                            userId: item.user.id,
                            userEmail: item.user.email,
                            userTelegram: item.user.telegramUsername,
                            isPremium: item.user.isPremium
                        }); }),
                        recentAlertsDetails: recentAlerts.map(function (alert) { return ({
                            id: alert.id,
                            walletAddress: alert.walletAddress,
                            chain: alert.chain,
                            transactionHash: alert.transactionHash,
                            type: alert.type,
                            value: alert.value,
                            tokenSymbol: alert.tokenSymbol,
                            tokenAmount: alert.tokenAmount,
                            createdAt: alert.createdAt,
                            userEmail: alert.user.email
                        }); })
                    };
                    console.log("\n\u2713 Monitoring complete:");
                    console.log("  - Wallets monitored: ".concat(results.walletsChecked));
                    console.log("  - Total alerts in system: ".concat(results.totalAlerts));
                    console.log("  - Recent alerts (24h): ".concat(results.recentAlerts));
                    return [2 /*return*/, results];
                case 5:
                    error_1 = _a.sent();
                    console.error("[".concat(timestamp, "] Monitoring failed:"), error_1.message);
                    return [2 /*return*/, {
                            timestamp: timestamp,
                            success: false,
                            error: error_1.message
                        }];
                case 6: return [4 /*yield*/, prisma.$disconnect()];
                case 7:
                    _a.sent();
                    return [7 /*endfinally*/];
                case 8: return [2 /*return*/];
            }
        });
    });
}
// Execute monitoring
monitorWatchlist()
    .then(function (result) {
    console.log("\n[".concat(new Date().toISOString(), "] Monitoring task completed"));
    // Output JSON for parsing
    console.log('\n=== JSON OUTPUT ===');
    console.log(JSON.stringify(result, null, 2));
    console.log('=== END JSON OUTPUT ===');
    process.exit(result.success ? 0 : 1);
})
    .catch(function (error) {
    console.error("\n[".concat(new Date().toISOString(), "] Fatal error:"), error);
    process.exit(1);
});
