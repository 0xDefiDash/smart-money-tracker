...
export async function GET(request: NextRequest) {
  // Updated fallback data for September 20, 2025
  const fallbackData = [
  ...
  try {
    // Try CoinGecko API to get top coins by volume
    ...
    console.info('Using fallback trending tokens data due to API limitations');
    ...
    return NextResponse.json({
      ...
      timestamp: new Date().toISOString(),
      fallback: true,
      message: 'Using fallback trending tokens data'
    });
  }
}
